...
// Draw a line from (xOld,yOld) to (xNew,yNew).
{
    ...
    glEnd();
    glFlush();
    Point start (xOld, yOld);
    Point end (xNew, yNew);
    Line line (start, end);
    drawn_lines.push_front(line);
}

void clearAndDrawPreviousLines()
{
    glClear(GL_COLOR_BUFFER_BIT);
...
int main( int argc, char * argv[] )
{
    ...
    glutDisplayFunc( clearAndDrawPreviousLines );
    glutMouseFunc( mouse );
